...
import utils
from hyperparameters import ALPHA, EPSILON, TRACKING_METHOD, LOAD_INSTEAD_OF_RECALCULATION, LOAD_MAX, DATASET_NAME
...
if __name__ == '__main__':
    ...
    node_change_file = open('datasets/' + DATASET_NAME + '-temporal/' + CHANGE_TYPE + '.txt', 'r')
    all_line = node_change_file.readlines()
    node_change_file.close()
    ...
    if not LOAD_INSTEAD_OF_RECALCULATION:
        ...
        sp.save_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/P_matrix_start.npz', P)
        np.save('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/h_array_start.npy', h)
    else:
        P = sp.load_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/P_matrix_start.npz')
        h = np.load('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/h_array_start.npy')
    ...
    utils.check_greedy_hit1(exp_match, ground_truth, nnodes_1)

    counter = 0
    for line in all_line:
        items = line.strip().split(',')
        change_node_in_sub = int(items[0])
        change_from = node_attr_matrix_1[change_node_in_sub, change_node_in_sub]
        change_to = int(items[1])
        counter += 1
        node_attr_matrix_1[change_node_in_sub, change_node_in_sub] = change_to
        node_attr_matrix_2[match_from_sub_to_whole(change_node_in_sub), match_from_sub_to_whole(change_node_in_sub)] = change_to
        node_attr_matrix_acce_1[change_node_in_sub, change_from] = 0
        node_attr_matrix_acce_1[change_node_in_sub, change_to] = 1
        node_attr_matrix_acce_2[match_from_sub_to_whole(change_node_in_sub), change_from] = 0
        node_attr_matrix_acce_2[match_from_sub_to_whole(change_node_in_sub), change_to] = 1
        print('change time: ', counter)

        if LOAD_INSTEAD_OF_RECALCULATION and (counter <= LOAD_MAX):
            P_new = sp.load_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/P_matrix_' + str(counter) + '.npz')
            h_new_sp = sp.load_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/h_array_sp_' + str(counter) + '.npz')
            ...
            sp.save_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/P_matrix_' + str(counter) + '.npz', P_new)
            h_new_sp = sp.csr_matrix(h_new)
            sp.save_npz('./' + CHANGE_TYPE + '_saved_data/' + DATASET_NAME + '/h_array_sp_' + str(counter) + '.npz', h_new_sp)
        time1 = time.time()
