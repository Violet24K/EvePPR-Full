...
if LOAD_INSTEAD_OF_RECALCULATION == 0:
    raise ValueError("If LOAD_INSTEAD OF RECALCULATION is 0, the generated matrix can be overwritten. Please run main.py to generate the transition matrices first and load instead of recalculating.")
...
if __name__ == '__main__':
    ...
    v[0] = utils.calc_ppr_by_power_iteration(P, ALPHA, h_uniform, 20)
    ...
    for line_index in range(start_line, end_line):
        ...
        if ABLATION == 0:
            h_new = h_uniform.copy()

        time1 = time.time()

        v[counter] = utils.osp(v[counter-1], P, P_new, ALPHA, EPSILON, 1)

        # v[counter] = utils.gauss_southwell(v[counter-1], P_new, h_new, ALPHA, EPSILON)
